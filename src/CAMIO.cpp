...
void CAMIO::CreateFile(const std::string& filePath) {
    ...
    blockList.push_back(GenerateBlock(CAMBlock::ACQP, header_size));
    blockList.push_back(GenerateBlock(CAMBlock::PROC, 
        static_cast<size_t>(header_size + static_cast<uint32_t>(BlockSize::ACQP))));

    // Create and place the lines in the blocks array
    size_t loc = static_cast<size_t>(header_size + 
        static_cast<uint32_t>(BlockSize::ACQP) +
        static_cast<uint32_t>(BlockSize::PROC));
    ...
    try {
        ...
        throw std::runtime_error(std::string("Failed to write file: ") + e.what());
    }
...
std::vector<byte_type> CAMIO::GenerateBlockHeader(CAMBlock block, size_t loc, uint16_t numRec,
    ...
    std::array<uint16_t, 20> values = {
        0x0500,                     // 0x04  0
        static_cast<uint16_t>(BlockSize::ACQP), // 0x06  1
        0x0000,                     // 0x08  2
        0x0000,                     // 0x0E  3
        0x0030,                     // 0x10  4
        0x0890,                     // 0x12  5
