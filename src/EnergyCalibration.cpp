...
namespace
{
  ...
  vector<SplineNode> create_cubic_spline( const vector<pair<float,float>> &xy )
  {
    ...
    for( size_t i = 1; i < n; ++i )
      alpha[i] = 3*(xy[i+1].second - xy[i].second)/h[i] - 3*(xy[i].second - xy[i-1].second)/h[i-1];
  
    for( size_t i = 1; i < n; ++i)
    {
      l[i] = 2*(xy[i+1].first - xy[i-1].first) - h[i-1]*mu[i-1];
      mu[i] = h[i] / l[i];
...
namespace SpecUtils
{
  ...
  void test_spline()
  {
    ...
    */
    ...
    cout << "GammaEnergy, NoDevPairEnergy, WithDevPair, AsBinning" << endl;
    for( size_t i = 0; i < ngammas; ++i )
    {
      ...
      cout << gamma_energies[i] << ", " << no_dev_pairs_peak_means[i] << ", " << corrected << ", " << (*from_apply_dev)[i] << endl;
    }
...
float offset_due_to_deviation_pairs( const float energy, const std::vector<std::pair<float,float>> &dps )
{
  if( dps.empty() )
    return 0.0;
