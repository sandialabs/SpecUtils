...
- **Windows**: You will need the CMake and node executables in your **PATH** variable.  Compiling has only been tested with MSVS-2017, but MSVS-2012 and newer should work. You will need to open the 'x64 Native Tools Command Propmpt' provided by MSVS, or execute the appropriate *vcvarsall.bat* to execute the steps outlined below.  Other compilers (clang, [MingGW](https://nuwen.net/mingw.html)) *should* work, but have not been tested.  Instructions for building boost can be found [here](https://www.boost.org/doc/libs/1_65_1/more/getting_started/windows.html). This project links against the static runtime libraries (e.g., uses the '/MT' flag), and statically links to boost.
...
- `SpecFile` This class represents a spectrum file on disk. Defined functions are:
  - `Constructor`: Takes a single *String* argument that gives the filesystem path to spectrum file to parse.
  - `sampleNumbers()`: Returns an *Array* of *Number*s containing all the sample numbers in the spectrum file.  Takes no arguments.
  - `detectorNames()`: Returns an *Array* of *String*s containing the detector names in the spectrum file. Takes no arguments.
  - `records( wantedDetectorNames, wantedSampleNumbers, wantedSourceTypes )`: Returns an array `SpecRecord` objects.  If provided, the arguments filters the types of `SpecRecord`s returned.
